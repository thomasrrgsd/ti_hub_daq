...
for f in min/*.html; do
    ...
    # Merge JS and HTML
    filename="${basename%.*}"
    python3.8 replaceInFile.py '<script src="js/'$filename'.js"></script>' min/js/$filename.js $f true
done
...
for f in min/dependencies/* min/images/* min/css/*.css min/*.html; do
    ...
    python3.8 byteConvert.py embedded/$basename.gz embedded/temp
    # Embed in output file
    python3.8 replaceInFile.py $templateKey embedded/temp $out
    # Cleanup
