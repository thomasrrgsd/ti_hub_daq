...
uint8_t command_buffer[25][6];
uint8_t command_buffer_index = 0;
uint8_t command_buffer_read = 0;
 ...
 * Checks for Channel 0 (UART0 TX) and channel 4 (UART2 TX).
 */
void DMA_INT0_IRQHandler(void){
    ...
    if(DMA_CH5_FLAG & DMA_getInterruptStatus()){
        DMA_clearInterruptFlag(5);
        //P2OUT ^= GPIO_PIN1;         // Toggle LED for debugging.
        // Here I need to tell uart that a command is recieved.
...
/*
 * DMA Interrupt 1 handler. This interrupts are setup in the DMA initialization.
